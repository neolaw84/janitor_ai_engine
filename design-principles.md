...
- **Script state update:** The script should update the `state` ([SCRIPT_SECRET]) based on the LLM's summary ([NARRATION_SUMMARY]). The following updates happen:
...
- **Creation of the guide:** The script should create a narration guide ([NARRATION_GUIDE]) based on the `state` ([SCRIPT_SECRET]) and the LLM's summary ([NARRATION_SUMMARY]). This guide contains two parts:
  - Part 1: How to update the `summary` ([NARRATION_SUMMARY]) based on what happens during the narration (the first design principle) and
...
- The developer should define the template of `summary` ([NARRATION_SUMMARY]) and for each of the entry (key) in the `summary`, define when and how the LLM is supposed to update it. I want to standardize the value of each entry (in the `script_def_template.js`) as an array of objects that looks like this:
...
- The developer should define an array of standardized functions that takes `state` ([SCRIPT_SECRET]) and `summary` ([NARRATION_SUMMARY]) as arguments and returns a string value. The `rollxdy` function will be available to the standardized functions defined as above. The returned string will be used to instruct the LLM on what to happen next as lines in the `narration_guide` ([NARRATION_GUIDE]) block. These functions can have side-effects to the `state` variables if needed.
