...
const context = {
    ...
    chat: {
        messages: [
            ...
            {
                role: "assistant",
                message: "Hi there. [TURN_SUMMARY]{\"elapsed_duration\": \"PT5M\", \"drink_alcohol\": [{\"what\": \"beer\", \"when\": \"2025-06-01T12:00:00Z\"}]}[/TURN_SUMMARY]"
            }
        ]
    }
};
...
try {
    ...
    console.log("Updated Scenario (Tail):", context.character.scenario.slice(-500));

    if (context.character.scenario.includes("[SCRIPT_SECRET]") && context.character.scenario.includes("[TURN_SUMMARY]")) {
